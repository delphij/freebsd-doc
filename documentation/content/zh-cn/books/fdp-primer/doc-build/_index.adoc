...
本章介绍文档构建过程的组织以及如何使用 man:make[1] 来控制它。
...
% make run
....
默认情况下，此 Web 服务器在 `localhost` 的 `1313` 端口上运行。

要使用 Hugo 的内部 Web 服务器绑定特定的 IP 地址和端口来提供内容：
...
也可以将 `hostname` 设置为 Hugo 内部 Web 服务器的基本 URL：
...
* 主要的构建工具是 man:make[1]，特别是 Berkeley Make。
* Hugo
* AsciiDoctor
* Git
...
有三个 [.filename]#Makefile# 文件用于构建部分或全部文档项目。

* [.filename]#documentation# 目录中的 [.filename]#Makefile# 只会构建文档。
* [.filename]#website# 目录中的 [.filename]#Makefile# 只会构建网站。
* 树顶部的 [.filename]#Makefile# 将同时构建文档和网站。

出现在子目录中的 [.filename]#Makefile# 也支持 `make run`，以使用 Hugo 的内部 Web 服务器提供构建的内容。
默认情况下，此 Web 服务器在端口 1313 上运行。

[[documentation-makefile]]
=== 文档 Makefile

这个 [.filename]#Makefile# 的形式如下：
...
starting-message: .PHONY <.>
	t@echo ---------------------------------------------------------------
	t@echo 			Building the documentation
	t@echo 	 included languages: ${LANGUAGES}
	t@echo 	 excluded languages: ${SKIP_LANGS}
	t@echo ---------------------------------------------------------------

generate-pgpkeys-txt: static/pgpkeys/pgpkeys.txt

static/pgpkeys/pgpkeys.txt:
...
<.> `MAINTAINER` 标志指定此 Makefile 的维护者。
<.> `ALL_LANGUAGES` 标志指定必须生成目录的语言。
<.> `RUBY_CMD` 标志指定 Ruby 二进制文件的位置。
<.> `HUGO_CMD` 标志指定 Hugo 二进制文件的位置。
<.> `.ORDER` 指令用于确保多个 make 作业可以无问题地运行。
<.> `all` 目标构建文档并将结果放入 *~/doc/documentation/public*。
<.> `starting-message` 在 CLI 中显示一条消息，以向用户显示该过程正在运行。
<.> `run-local` 在端口 1313 上运行 hugo Web 服务器，如果该端口已被使用，则使用一个随机的空闲端口。
<.> `build` 构建文档并将结果放入 *~/doc/documentation/public*。
...
<.> `HUGO_CMD` 标志指定 Hugo 二进制文件的位置。
<.> `.ORDER` 指令用于确保多个 make 作业可以无问题地运行。
<.> `all` 目标构建网站并将结果放入 *~/doc/website/public*。
<.> `starting-message` 在 CLI 中显示一条消息，以向用户显示该过程正在运行。
<.> `generate-releases` 调用用于将 AsciiDoc 变量转换为 TOML 变量的脚本。
通过此转换，版本变量可以在 AsciiDoc 和 Hugo 自定义模板中使用。
<.> `run-local` 在端口 1313 上运行 hugo Web 服务器，如果该端口已被使用，则使用一个随机的空闲端口。
<.> `build` 构建网站并将结果放入 *~/doc/website/public*。
